...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=None):
    ...
    croped_pil_image = []
    valid_indices = []  # Track which boxes yielded valid images
    ...
    for i, coord in enumerate(non_ocr_boxes):
        try:
            ...
            elif cropped_image.shape[-1] != 3:
                print(f"Warning: Unexpected image shape {cropped_image.shape}, skipping")
                continue
                
            cropped_image = cv2.resize(cropped_image, (64, 64))
            # Convert numpy array to PIL Image in RGB mode
            pil_image = Image.fromarray(cropped_image).convert('RGB')
            croped_pil_image.append(pil_image)
            valid_indices.append(i)  # Keep track of which boxes we successfully processed
    ...
    if not croped_pil_image:
        ...
        return [""] * len(non_ocr_boxes)  # Return empty captions for all boxes
    ...
    for i in range(0, len(croped_pil_image), batch_size):
        ...
        if model.device.type == 'cuda':
            inputs = processor(images=batch, text=[prompt]*len(batch), return_tensors="pt", do_resize=True).to(device=device, dtype=torch.float16)
        else:
            inputs = processor(images=batch, text=[prompt]*len(batch), return_tensors="pt", do_resize=True).to(device=device)
    ...
    # Create a list of captions for all boxes, with empty strings for skipped boxes
    all_captions = [""] * len(non_ocr_boxes)
    for idx, caption in zip(valid_indices, generated_texts):
        all_captions[idx] = caption
...
def get_som_labeled_img(image_source: Union[str, Image.Image], model=None, BOX_TRESHOLD=0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=64):
    """Process either an image path or PIL Image object
    
    Args:
        image_source: Either a file path (str) or PIL Image object
        ...
    Returns:
        Tuple of (encoded_image, label_coordinates, filtered_boxes_elem)
        where filtered_boxes_elem is a list of dicts with keys:
    ...
    if use_local_semantics:
        ...
        for i, box in enumerate(filtered_boxes_elem):
            if box['content'] is None:
                box['content'] = parsed_content_icon[i - starting_idx] if i >= starting_idx else ""
                if box['content']:  # Only add non-empty captions to the list
                    parsed_content_icon_ls.append(f"Icon Box ID {str(i+icon_start)}: {box['content']}")
    ...
    for box in filtered_boxes_elem:
        ...
        box['interactivity'] = bool(box['interactivity'])
        box['content'] = str(box['content']) if box['content'] else ""
